...
{
    public static final int ABS = 0, ACOS = 1, ASIN = 2, ATAN = 3, ATAN2 = 4, BITAND = 5,
            BITOR = 6, BITXOR = 7, CEILING = 8, COS = 9, COT = 10, DEGREES = 11, EXP = 12,
            FLOOR = 13, LOG = 14, LOG10 = 15, MOD = 16, PI = 17, POWER = 18, RADIANS = 19,
            RAND = 20, ROUND = 21, ROUNDMAGIC = 22, SIGN = 23, SIN = 24, SQRT = 25,
            TAN = 26, TRUNCATE = 27, SECURE_RAND = 28, HASH = 29, ENCRYPT = 30,
            DECRYPT = 31, COMPRESS = 32, EXPAND = 33, ZERO = 34, RANDOM_UUID = 35;
    ...
    public static final int ASCII = 50, BIT_LENGTH = 51, CHAR = 52, CHAR_LENGTH = 53,
            ...
            LCASE = 59, LEFT = 60, LENGTH = 61, LOCATE = 62, LTRIM = 63, OCTET_LENGTH = 64,
            RAWTOHEX = 65, REPEAT = 66, REPLACE = 67, RIGHT = 68, RTRIM = 69, SOUNDEX = 70,
            SPACE = 71, SUBSTR = 72, SUBSTRING = 73, UCASE = 74, LOWER = 75, UPPER = 76,
            POSITION = 77, TRIM = 78, STRINGENCODE = 79, STRINGDECODE = 80,
            STRINGTOUTF8 = 81, UTF8TOSTRING = 82, XMLATTR = 83, XMLNODE = 84,
    ...
    public static final int CURDATE = 100, CURTIME = 101, DATE_ADD = 102, DATE_DIFF = 103,
            DAY_NAME = 104, DAY_OF_MONTH = 105, DAY_OF_WEEK = 106, DAY_OF_YEAR = 107,
            HOUR = 108, MINUTE = 109, MONTH = 110, MONTH_NAME = 111, NOW = 112,
            QUARTER = 113, SECOND = 114, WEEK = 115, YEAR = 116, CURRENT_DATE = 117,
            CURRENT_TIME = 118, CURRENT_TIMESTAMP = 119, EXTRACT = 120,
            FORMATDATETIME = 121, PARSEDATETIME = 122, ISO_YEAR = 123, ISO_WEEK = 124,
            ISO_DAY_OF_WEEK = 125;
    
    public static final int DATABASE = 150, USER = 151, CURRENT_USER = 152, IDENTITY = 153,
            AUTOCOMMIT = 154, READONLY = 155, DATABASE_PATH = 156, LOCK_TIMEOUT = 157;
    ...
    static
    {
        ...
        // Lines starting with "// --> " are funcitons commented out by javawizard because
        // they aren't safe for remote access. It'd be nice to have a connection parameter
        // that can be used to specify whether these are disabled. Consider submitting
        // this as a patch to the H2 guys.
        ...
        addFunctionNotDeterministic("CURRENT_TIMESTAMP", CURRENT_TIMESTAMP, VAR_ARGS,
                Value.TIMESTAMP);
    ...
    private static void addFunctionWithNull(String name, int type, int parameterCount,
            int dataType)
    {
    ...
    /**
     * Get the function info object for this function, or null if there is no such
     * function.
     * 
     * @param name
     *            the function name
    ...
    /**
     * Get an instance of the given function for this database. If no function with this
            ...
            throws SQLException
    {
        ...
        switch (info.type)
        {
            ...
            case EXPAND:
                result =
                        ValueBytes.getNoCopy(CompressTool.getInstance().expand(
                                v0.getBytesNoCopy()));
                break;
            case ZERO:
            ...
            case DAY_OF_MONTH:
                result =
                        ValueInt.get(DateTimeUtils.getDatePart(v0.getDateNoCopy(),
            ...
            case HOUR:
                result =
                        ValueInt.get(DateTimeUtils.getDatePart(v0.getTimestampNoCopy(),
            ...
            case ISO_DAY_OF_WEEK:
                result =
                        ValueInt.get(DateTimeIso8601Utils.getIsoDayOfWeek(v0
                                .getDateNoCopy()));
                break;
            case CURDATE:
            case CURRENT_DATE:
            ...
            {
                ValueTimestamp vt =
                        ValueTimestamp.getNoCopy(new Timestamp(System.currentTimeMillis()));
                if (v0 != null)
                {
                    Mode mode = database.getMode();
                    vt =
                            (ValueTimestamp) vt.convertScale(
            ...
            case CASE:
            {
                ...
                {
                    result =
                            i < args.length ? args[i].getValue(session)
                                    : ValueNull.INSTANCE;
                }
                break;
            }
    ...
    private Value getValueWithArgs(Session session, Expression[] args) throws SQLException
    {
        ...
        switch (info.type)
        {
            ...
            case HASH:
                result =
                        ValueBytes.getNoCopy(getHash(v0.getString(), v1.getBytesNoCopy(),
                                v2.getInt()));
                break;
            case ENCRYPT:
                result =
                        ValueBytes.getNoCopy(encrypt(v0.getString(), v1.getBytesNoCopy(),
                                v2.getBytesNoCopy()));
                break;
            case DECRYPT:
                result =
                        ValueBytes.getNoCopy(decrypt(v0.getString(), v1.getBytesNoCopy(),
                                v2.getBytesNoCopy()));
                break;
            case COMPRESS:
            {
                ...
                result =
                        ValueBytes.getNoCopy(CompressTool.getInstance().compress(
            ...
            case INSERT:
            {
                ...
                {
                    result =
                            ValueString.get(insert(v0.getString(), v1.getInt(),
            ...
            case LTRIM:
                result =
                        ValueString.get(StringUtils.trim(v0.getString(), true, false,
                                v1 == null ? " " : v1.getString()));
                break;
            case TRIM:
                result =
                        ValueString.get(StringUtils.trim(v0.getString(), true, true,
                                v1 == null ? " " : v1.getString()));
                break;
            case RTRIM:
                result =
                        ValueString.get(StringUtils.trim(v0.getString(), false, true,
                                v1 == null ? " " : v1.getString()));
                break;
            case SUBSTR:
            case SUBSTRING:
            ...
            case XMLATTR:
                result =
                        ValueString
                                .get(StringUtils.xmlAttr(v0.getString(), v1.getString()));
                break;
            case XMLNODE:
            {
                String attr =
                        v1 == null ? null : v1 == ValueNull.INSTANCE ? null : v1
                                .getString();
                String content =
                        v2 == null ? null : v2 == ValueNull.INSTANCE ? null : v2
                                .getString();
                result =
                        ValueString.get(StringUtils.xmlNode(v0.getString(), attr, content));
                break;
            }
            case REGEXP_REPLACE:
            {
                ...
                {
                    result =
                            ValueString.get(v0.getString().replaceAll(regexp,
                                    v2.getString()));
                }
            ...
            case RPAD:
                result =
                        ValueString.get(StringUtils.pad(v0.getString(), v1.getInt(),
            ...
            case DATE_ADD:
                result =
                        ValueTimestamp.getNoCopy(dateadd(v0.getString(), v1.getInt(), v2
                                .getTimestampNoCopy()));
                break;
            case DATE_DIFF:
                result =
                        ValueLong.get(datediff(v0.getString(), v1.getTimestampNoCopy(), v2
                                .getTimestampNoCopy()));
                break;
            case EXTRACT:
            {
                ...
                else
                {
                    String locale =
                            v2 == null ? null : v2 == ValueNull.INSTANCE ? null : v2
                                    .getString();
                    String tz =
                            v3 == null ? null : v3 == ValueNull.INSTANCE ? null : v3
                                    .getString();
                    result =
                            ValueString.get(StringUtils.formatDateTime(v0.getTimestamp(),
                ...
                else
                {
                    ...
                    String tz =
                            v3 == null ? null : v3 == ValueNull.INSTANCE ? null : v3
                                    .getString();
                    java.util.Date d =
                            StringUtils.parseDateTime(v0.getString(), v1.getString(),
                                    locale, tz);
            ...
            case CSVREAD:
            {
                ...
                ValueResultSet vr =
                        ValueResultSet.get(csv.read(fileName, columns, charset));
                result = vr;
                break;
            }
            case LINK_SCHEMA:
            {
                ...
                ResultSet rs =
                        LinkSchema.linkSchema(conn, v0.getString(), v1.getString(), v2
                                .getString(), v3.getString(), v4.getString(), v5
                                .getString());
            ...
            case FILE_READ:
            {
                ...
                {
                    InputStream in =
                            new AutoCloseInputStream(FileUtils
                                    .openFileInputStream(fileName));
                    if (blob)
                    {
            ...
            throws SQLException
    {
        ...
        calendar.setTime(d1);
        t1 +=
                zone.getOffset(calendar.get(Calendar.ERA), calendar.get(Calendar.YEAR),
    ...
    /**
     * This method is called after all the parameters have been set. It checks if the
    ...
    public Expression optimize(Session session) throws SQLException
    {
        ...
        switch (info.type)
        {
            ...
            case SET:
            {
                ...
                {
                    throw Message.getSQLException(ErrorCode.CAN_ONLY_ASSIGN_TO_VARIABLE_1,
                            p0.getSQL());
                }
                break;
            }
        ...
        switch (info.type)
        {
            ...
            case CONCAT:
                ...
                for (Expression e : args)
                {
                    ...
                    displaySize =
                            MathUtils.convertLongToInt((long) displaySize
                                + e.getDisplaySize());
                    if (precision < 0)
                    {
    ...
    public String getSQL()
    {
        ...
        switch (info.type)
        {
            case CAST:
            {
                buff.append(args[0].getSQL()).append(" AS ").append(
                        new Column(null, dataType, precision, scale, displaySize)
                                .getCreateSQL());
                break;
            }
            case CONVERT:
            {
                buff.append(args[0].getSQL()).append(',').append(
                        new Column(null, dataType, precision, scale, displaySize)
                                .getCreateSQL());
                break;
            }
    ...
    public ValueResultSet getValueForColumnList(Session session, Expression[] args)
            throws SQLException
    {
        switch (info.type)
        {
            case CSVREAD:
            {
                ...
                {
                    throw Message
                            .getSQLException(ErrorCode.PARAMETER_NOT_SET_1, "fileName");
                }
                String columnList =
                        args.length < 2 ? null : args[1].getValue(session).getString();
                String charset =
                        args.length < 3 ? null : args[2].getValue(session).getString();
                String fieldSeparatorRead =
                        args.length < 4 ? null : args[3].getValue(session).getString();
                String fieldDelimiter =
                        args.length < 5 ? null : args[4].getValue(session).getString();
                String escapeCharacter =
                        args.length < 6 ? null : args[5].getValue(session).getString();
                Csv csv = Csv.getInstance();
                setCsvDelimiterEscape(csv, fieldSeparatorRead, fieldDelimiter,
                        escapeCharacter);
                char fieldSeparator = csv.getFieldSeparatorRead();
